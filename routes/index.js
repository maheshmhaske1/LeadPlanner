...
router.get('/', (req, res) => {
    ...
    return Array.from(document.querySelectorAll(".Nv2PK")).map((el) => {
        ...
        const addressElements = el.querySelector(".W4Efsd:last-child > .W4Efsd:nth-of-type(1)").querySelectorAll('span');

      // Extract city, state, and pincode from the address
      const city = addressElements[0]?.textContent.trim();
      const state = addressElements[1]?.textContent.trim();
      const pincode = addressElements[2]?.textContent.trim()
